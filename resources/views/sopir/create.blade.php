{{-- resources/views/sopir/create.blade.php --}}
<x-app-layout>
    <div class="py-8">
        <div class="mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white border dark:bg-gray-800 sm:rounded-lg p-6 shadow">
                <form action="{{ route('sopir.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input id="nama_sopir" name="nama_sopir" type="text" value="{{ old('nama_sopir') }}"
                        ...
                        <div>
                            ...
                            <input id="nomor_ktp" name="nomor_ktp" type="text" value="{{ old('nomor_ktp') }}"
                                required placeholder="Masukkan nomor KTP"
                                class="block w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200
                         focus:ring-2 focus:ring-green-500 focus:border-green-500 rounded-lg p-2 transition" />
                        </div>
                        <div>
                            ...
                            <input id="sim" name="sim" type="text" value="{{ old('sim') }}"
                        ...
                        <div>
                            ...
                            <input id="umur" name="umur" type="number" value="{{ old('umur') }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="alamat" name="alamat" rows="3" placeholder="Masukkan alamat lengkap"
                                class="block w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200
                         focus:ring-2 focus:ring-green-500 focus:border-green-500 rounded-lg p-2 transition">{{ old('alamat') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                        {{-- Foto Input --}}
                        <div>
                            <label for="foto-create" class="block font-medium text-gray-700 dark:text-gray-300 mb-1">
                                Foto Sopir (opsional)
                            </label>
                            <input id="foto-create" name="foto" type="file" accept="image/*"
                                class="block w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200
                         focus:ring-2 focus:ring-green-500 focus:border-green-500 rounded-lg p-2 transition" />
                        </div>
                        {{-- Preview --}}
                        <div>
                            ...
                            <img id="preview-foto-create" src="#" alt="Preview Foto"
                                class="hidden w-full h-auto max-w-xs object-cover rounded-md border" />
                        </div>
                    </div>
    ...
    {{-- Preview Script --}}
    <script>
        document.getElementById('foto-create').addEventListener('change', function(e) {
            const file = e.target.files[0];
            const img = document.getElementById('preview-foto-create');
            if (!file) {
                img.src = '#';
